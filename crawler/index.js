 ...
 * TODO: 爬取页面
 */
superagent.get(URL).charset().buffer(true).then((data) => {
  ...
  $('.shot-thumbnail').each(async function (i, element) {
    ...
    const thumbnail=$element.find('img').attr('src')||$element.find('video').attr('src')
    // 下载缩略图
    download(thumbnail)
...
function getImageNode(pageUrl){
  superagent.get(pageUrl).charset().buffer(true).then((singleData) => {
    // TODO:根据单个单元获取产品图片列表
    const $ = cheerio.load(singleData.text)
    const name = $('.shot-title').text()
    // const imgHrefList = []
    $('.media-content').each( async(index, node) => {
      const $node = $(node)
      const herf= $node.find('img').attr('data-src') || $node.find('video').attr('data-src')
